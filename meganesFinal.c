...
#define EPSILON 0.000001 //ε 小さい正の値
...
here:
    ...
    double distance = tour_length(p, n, tour);
    if ((distance - cur_distance) > EPSILON) 
        printf("Swap changed = %lf\n", distance);
}
...
here:
    ...
    // }
    double distance_tour  = tour_length(p, n, tour);
    if (distance_tour  + EPSILON < cur_distance) {
        printf("2-opt changed = %lf\n", distance_tour);
    }
}
	...
	while(count>0){
      ...
      double distance_tour = tour_length(p, n, tour);
        if ((cur_distance - distance_tour) > EPSILON) {
            ...
            cur_distance = distance_tour;
            change++;
            //f ((min_distance - cur_distance) > EPSILON) {
            //    min_distance = cur_distance;
            //    write_tour_data(tourFileName, n, tour);
            //}
            goto here;
...
void OrOpt3(struct point p[MAX_N],int n,int tour[MAX_N],int m, int prec[MAX_N]){
  int a,b,c,d,e,f,g;
  int i0, i, i1, i2, i3, j, j1, tmp, tmp1, tmp2, count;
...
void call(int f_i, struct point p[MAX_N], int n,int tour[MAX_N],int m, int prec[MAX_N]) {
    switch (f_i) {
        case 0:
            printf("2-Opt!\n");
