...
#define EPSILON 0.00001  //ε 小さい正の値
#define SWAP(a, b)  \
    {               \
        int temp;   \
        temp = (a); \
...
void check(int tour[], int size_tour, int prec[], int number_prec) {
    // printf("Check!\n");
    int positions[size_tour];
    ...
    for (int i = 1; i < number_prec; ++i) {
        assert(positions[prec[i]] > positions[prec[i - 1]]);
    }
}
...
void TwoOpt(struct point p[], int n, int tour[], int m, int prec[]) {
    double start_distance = tour_length(p, n, tour);
    ...
    if (start_distance > cur_distance + EPSILON) {
        change++;
        printf("Changed by 2-opt = %lf\n", cur_distance);
        if (S_tour > cur_distance) {
            S_tour = cur_distance;
